...
q_list = [x / 100 for x in [0.1, 0.5, 1.0, 5.0, 10.0]]

instances = ['Atlanta', 'SanFrancisco']


def grtd_out(in_dir: str, out_dir: str, alg: list, run: bool):
    ...
    for _alg in alg:
        for instance in instances:
            ...
            ax.set_ylabel("Number of infected nodes", fontweight="bold")
            for q in q_list:
                mask = time_qual[time_qual['rel_qual'] <= q]
                ax.step(mask['_time'], np.arange(1, mask.shape[0] + 1) / mask.shape[0],
